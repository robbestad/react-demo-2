!function(){"use strict"}();
var Mouse=React.createClass({displayName:"Mouse",getDefaultProps:function(){return{initialPos:{x:0,y:0}}},getInitialState:function(){return{pos:this.props.initialPos,rel:null}},componentDidUpdate:function(){document.addEventListener("mousemove",this.onMouseMove),document.addEventListener("touchstart",this.onTouchMove),document.addEventListener("touchend",this.onTouchMove),document.addEventListener("touchcancel",this.onTouchMove),document.addEventListener("touchmove",this.onTouchMove)},onMouseMove:function(e){this.setState({pos:{x:e.pageX-50,y:e.pageY-50}}),e.stopPropagation(),e.preventDefault()},onTouchMove:function(e){this.setState({pos:{x:e.pageX-50,y:e.pageY-50}}),e.stopPropagation(),e.preventDefault()},render:function(){return this.transferPropsTo(React.DOM.div({onMouseMove:this.onMouseMove,onTouchMove:this.onTouchMove,style:{position:"absolute",left:this.state.pos.x+"px",top:this.state.pos.y+"px"}},this.props.children))}});React.renderComponent(Mouse({initialPos:{x:100,y:200},className:"mouse",style:{border:"1px solid #aa5",background:"green",color:"white",padding:"10px",width:"100px",height:"100px",position:"absolute"}},"Flytt meg"),document.body);
